...
const JoinClubCoding = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/coding-join', {
        fullName,
        email,
        studentId,
        department,
      ...
      if (response.status === 200) {
        setHasJoined(true);
        navigate('/coding'); // Redirect to sports dashboard
      }
    } catch (error) {
