...
const JoinClubSports = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/sports-join', {
        fullName,
        email,
        studentId,
        department,
      ...
      if (response.status === 200) {
        setHasJoined(true);
        navigate('/sports'); // Redirect to sports dashboard
      }
    } catch (error) {
